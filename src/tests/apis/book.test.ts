import { Book } from '@src/types/apis/book.d';
import { getBookList, getBookItem } from '@src/apis/book';

describe('Book API', () => {
  test('getBookList', async () => {
    const keyword: string = '한강';
    const data: Book[] = await getBookList(keyword);

    expect(data).toContain<Book>({
  ...
  test('getBookItem', async () => {
    const isbn13: number = 9788954693462;
    const data: Book = await getBookItem(isbn13);
